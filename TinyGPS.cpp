...
bool TinyGPS::term_complete()
{
    ...
    switch(COMBINE(_sentence_type, _term_number))
  {
    ...
    case COMBINE(_GPS_SENTENCE_GPGGA, 1):
      ...
      _new_time_fix = to_us_since_boot(get_absolute_time());
      break;
    ...
    case COMBINE(_GPS_SENTENCE_GPGGA, 2):
      ...
      _new_position_fix = to_us_since_boot(get_absolute_time());
      break;
...
void TinyGPS::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
  ...
  if (fix_age) *fix_age = _last_position_fix == GPS_INVALID_FIX_TIME ? 
   GPS_INVALID_AGE : to_us_since_boot(get_absolute_time()) - _last_position_fix;
}
...
void TinyGPS::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  ...
  if (age) *age = _last_time_fix == GPS_INVALID_FIX_TIME ? 
   GPS_INVALID_AGE : to_us_since_boot(get_absolute_time()) - _last_time_fix;
}
...
void TinyGPS::crack_datetime(int *year, char *month, char *day, 
  char *hour, char *minute, char *second, char *hundredths, unsigned long *age)
{
