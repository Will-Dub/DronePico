...
const uint MIN_WIDTH = 650; // in microseconds
const uint MAX_WIDTH = 2400; // in microseconds

class TurnigyESC {
public:
    TurnigyESC(uint pin) : pin_(pin) {
        set_pin_function(pin_);
        setup_pwm();
    }

    void pwm(uint width, uint snooze = 0) {
        uint16_t pulse_width = map_width_to_pwm(width);
        pwm_set_chan_level(pwm_slice_num_, PWM_CHAN_A, pulse_width);
        sleep_ms(snooze * 1000);
    }

    void calibrate() {
        printf("Calibrating...\n");
        pwm(MAX_WIDTH);
        printf("Connect power and press Enter to calibrate...\n");
        sleep_ms(10000);
        pwm(MAX_WIDTH, 2); // Wait for 2 seconds
        pwm(MIN_WIDTH, 4); // Wait for 4 seconds
    ...
    void halt() {
        printf("Slowing...\n");
        pwm(MIN_WIDTH, 1); // Wait for 1 second
        printf("Failsafe...\n");
        pwm(0);
    ...
    void test() {
        printf("Press Enter to conduct run-up test...\n");
        sleep_ms(10000);

        uint max_width = MAX_WIDTH - 1330; // in microseconds
        uint min_width = MIN_WIDTH + 80; // in microseconds
        uint step = 9; // in microseconds
        uint snooze = 300; // in milliseconds

        printf("Increasing...\n");
        for (uint width = min_width; width <= max_width; width += step) {
            pwm(width, snooze / 1000);
        }

        sleep_ms(1000); // Duration test

        printf("Holding at max...\n");
        snooze = 100; // in milliseconds

        printf("Decreasing...\n");
        for (uint width = max_width; width >= min_width; width -= step) {
            pwm(width, snooze / 1000);
        }
    }
...
private:
    ...
    void set_pin_function(uint pin) {
        gpio_set_function(pin, GPIO_FUNC_PWM);
    }

    void setup_pwm() {
        pwm_slice_num_ = pwm_gpio_to_slice_num(pin_);
        pwm_set_wrap(pwm_slice_num_, 4095); // Set PWM wrap value
        pwm_set_chan_level(pwm_slice_num_, PWM_CHAN_A, 0); // Set initial PWM level to 0
        pwm_set_enabled(pwm_slice_num_, true); // Enable PWM
    }

    uint16_t map_width_to_pwm(uint width) {
        return (width * 4095) / MAX_WIDTH; // Map width to PWM range
    }
};
...
void controlMotors(UART* uart_zero){
    ...
    while (true) {
        ...
        if (data_ready) {
            ...
            controlData.pitch = 0;
            controlData.roll = 10;
            controlData.throttle = 20;
            controlData.yaw = 40;

            Message messagef;
            messagef.type = MessageType::ControlData;
            messagef.data.control_data = controlData;
            uart_zero->writeMessage(messagef);
        ...
        std::optional<Message> received_message_opt = uart_zero->getReceiveMessage();
        mutex_exit(&zero_mutex);

        if (received_message_opt.has_value()) {
            Message received_message = received_message_opt.value();
            log(uart_zero, "NEW DATA!!!!!!!", &zero_mutex, LogType::LOG_INFO);
            sleep_ms(10);
        }
...
int main() {
    ...
    log(&uart_zero, "Start", &zero_mutex);
    sleep_ms(5000);
    stdio_init_all();

    TurnigyESC esc(5);

    log(&uart_zero, "Calibrating...", &zero_mutex);
    esc.calibrate();
    log(&uart_zero, "Arming...", &zero_mutex);
    esc.arm();
    log(&uart_zero, "Testing...", &zero_mutex);
    esc.test();
    log(&uart_zero, "Halting...", &zero_mutex);
    esc.halt();
    log(&uart_zero, "End", &zero_mutex);
